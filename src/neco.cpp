...
auto Neco::setPosNeco() -> void { sprite.setPosition(x, y); }

void Neco::burenyaa() { sound.play(); }

sf::Sprite &Neco::draw() { return sprite; }

auto Neco::showText(sf::Vector2f coords) -> void {
  std::string wcoords = "Window coords = (" + std::to_string(int(coords.x)) +
                        ", " + std::to_string(int(coords.y)) + ")\n";
  std::string xy = "(x, y) = (" + std::to_string(int(x)) + ", " +
                   std::to_string(int(y)) + ")\n";
  std::string edgec = "Edge Count = " + std::to_string(edgeCount) + "\n";
  text.setString(wcoords + xy + edgec);
}

void Neco::setIsTextHidden(bool val) { isTextHidden = val; }

bool Neco::getIsTextHidden() { return isTextHidden; }

sf::Text &Neco::drawText() { return text; }